...
def calculate_monthly_asset_value(filepath):
    ...
    ticker_map = df['股票代號'].apply(str).to_dict()
    grouped['Yahoo代碼'] = grouped['股票代號']

    price_data = {}
    for code in df['股票代號'].unique():
        try:
            ...
            price_data[code] = pd.Series(dtype=float)
    ...
    def get_price(row):
        ...
        month = row['月份']
        try:
            return row['累計股數'] * price_df.at[month, ticker]
        except:
            return 0.0
    ...
    def market_split(df, market_type):
        return df[df['Yahoo代碼'].map(market_map) == market_type].groupby(['月份', '擁有者'])['市值'].sum().unstack(fill_value=0)
    ...
    return summary_df, detail_df, df, monthly_Lo, monthly_Sean, monthly_Joint, price_df
